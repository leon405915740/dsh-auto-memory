...
class MemoryEngine {
   ...
   *  与增改留痕的硬层(覆写/删除都在段内留痕完成),只有日期流水段(History)才是压缩对象。 */
  ...
  renderMemoryDynamic(context) {
    ...
    // 项目笔记四层注入:Current State(唯一权威)→ Constraints(增改留痕)→ Lessons(增改留痕)→ 历史流水(append)
    ...
    if (consText) part('项目 Constraints(增改留痕)——不可偏离的底线;覆写/删除均留痕', consText, sub)
    if (lesText) part('项目 Lessons(增改留痕)——犯过的错、学到的经验', lesText, sub)
  ...
  renderMemoryStatic() {
    ...
    lines.push('- **踩坑留痕铁律(硬规则)**:凡当轮发生过——①命令/操作失败后经重试或补救成功;②被用户纠正错误(时间点/事实/做法);③需二次动作才完成的任务——必须在同一轮内把"坑+标准动作(含具体命令/路径/端口)"写入项目 Lessons(layer=lesson),跨项目通用的写用户级记忆,禁止"只修不记"。记录格式:日期+坑+标准动作。')
    lines.push('- **动手前先查教训(硬规则,与踩坑留痕配对)**:执行关键操作(git push/发版/上传仓库;修改记忆文件;修改插件或代码;网络/代理/外部服务/跨机安装;曾失败或重试过的同类任务)前,必须先 memory_recall 检索 Lessons/用户级记忆中的相关教训,命中则按标准动作执行。与留痕铁律构成闭环:先查(读)→执行→踩坑则当轮留痕(写)。')
  ...
  // ---------- 项目笔记四层结构(agent-memory-management 思想,SKILL v2 增改留痕口径) ----------
  // 四层判据:现状进 Current State(唯一权威,覆盖更新),规则进 Constraints(增改留痕),
  // 教训进 Lessons(增改留痕),推理/决策过程进 History(每日日志 append-only)。
  // 文件布局(项目笔记 MEMORY.md): 三个固定层段 + 日期流水段(兼容旧数据):
  //   ## Current State(唯一权威,覆盖更新)   ← 覆盖写 + 时间戳
  //   ## Constraints(增改留痕)               ← 三查裁决后写入:追加[active]/覆写留痕[superseded]
  //   ## Lessons(增改留痕)                   ← 同上
  //   ## YYYY-MM-DD ...                      ← 历史流水(append),memory_note 常规追加仍走这里
  // 兼容:旧版标题"Constraints(只增不改)/Lessons(只增不改)"按前缀正则同样识别,不丢数据。
  ...
  /**
   * 向 Constraints / Lessons 段追加要点(增改留痕)。段不存在则自动创建。
   * 写入查重合并:①与已有条目完全重复(≥0.995)→ 跳过;②近似重复(≥0.72)→ 旧条目归档、新条目替换上位;
   * ③同实体取值变化 → 覆写留痕(旧条原位标注 [superseded]);④其他 → 追加 [active]。近似重复的旧条目移入 archive/(可追溯)。
   */
  async appendToNoteLayer(agent, kind, lines) {
    const p = await this.resolvePaths(agent)
    const cur = this.state.notesText || (await this.readTextSafe(p.notesPath)) || ''
    const { head, layers } = this.parseNoteLayers(cur)
    const title = kind === 'constraints' ? 'Constraints(增改留痕)' : 'Lessons(增改留痕)'
    let seg = layers[kind]
  ...
  /**
   * 四层段维护(淘汰 + 去重,防无限膨胀)。
   * 原则:「增改留痕」是读写纪律,不是无限增长许可——活段设容量上限(layerMaxEntries/layerMaxChars),
    ...
    this._consolidating = (async () => {
      try {
        ...
        const prompt = [
          ...
          '- **强制检查(踩坑留痕铁律)**:扫描本轮内容,若出现——①命令/操作失败后重试或补救成功;②用户纠正了错误(时间点/事实/做法);③需要二次动作才完成的任务——**必须**提炼 1 条 [LESSON](坑+标准动作,含具体命令/路径/端口),不得遗漏,不许只输出 (无)。本轮无此情形则不输出 [LESSON]。',
          '输出格式(严格遵守):',
          '[TOPIC]',
  ...
  async maintain(days = 30, agent, force) {
    ...
    this.state.loadedAt = Date.now()
    // 5) 层段 AI 梳理:Constraints/Lessons 里过时条目(被新规则取代/已失效/重复已久)删除并归档(防"增改留痕"变成垃圾堆积)
    let staleMsg = ''
    try {
      if (!forced && this._subagents && agent) {
  ...
  async pruneStaleLayers(agent) {
    ...
    for (const kind of targetKinds) {
      ...
      const prompt = [
        '你是记忆整理员。下面是项目记忆里「' + (kind === 'constraints' ? 'Constraints(硬规则,增改留痕)' : 'Lessons(教训,增改留痕)') + '」段的全部条目。',
...
export function apply(ctx, config) {
  ...
  const tools = [
    ...
    defineTool('memory_note', '更新当前项目长期笔记 MEMORY.md(本项目专属的约定、决策、架构要点)。四层用法:layer=state 覆盖 Current State 段(唯一权威,更新现状/方案,旧方案直接覆盖不堆历史);layer=constraint 追加 Constraints 段(硬规则/底线);layer=lesson 追加 Lessons 段(犯过的错/经验);layer=history(缺省)追加带日期标题的流水段。**生命周期自动裁决(增改留痕)**:写入前插件按三查裁决——同义/近义重复 → 合并去重(保留最全最新,旧条归档);同一实体取值变化(如版本升级/取值被推翻)→ 覆写留痕(旧值原位标注 ~~旧值~~ [superseded] 已覆写为:新值+日期原因);全新事实 → 追加 [active];陈旧/失效条目由 7 天自动维护软删留痕。每日预算 3000 字/天,超限自动压缩旧内容腾空间(四层段永不压缩,只压缩日期流水)。**调用后必须在本轮回复正文中向用户转述:更新了项目笔记、加入什么要点**。', {
      ...
      layer: { type: 'string', enum: ['history', 'state', 'constraint', 'lesson'], description: '写入层,缺省 history=日期流水段;state=覆盖 Current State;constraint/lesson=增改留痕的硬层段(写入即三查裁决:追加[active]/覆写留痕[superseded]/合并去重)。' },
    }, async (args, exec) => {
      const p = await engine.resolvePaths(exec.agent)
      ...
      // ← SKILL 四层:state 覆盖 Current State(唯一权威);constraint/lesson 增改留痕(写入即三查裁决);history 走原 append
